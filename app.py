...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    if not item:
        return "No listing was selected. Please try another search.", "", ""

    brand = item.get("brand") or "Unbranded"
    colors = ", ".join(item.get("colors", [])) or "Not specified"
    price = item.get("price")
    price_text = f"${price:.2f}" if isinstance(price, (int, float)) else "Not listed"
    listing_text = (
        f"{item.get('title', 'Untitled listing')}\n"
        f"Price: {price_text}\n"
        f"Size: {item.get('size', 'Not specified')}\n"
        f"Condition: {item.get('condition', 'Not specified')}\n"
        f"Brand: {brand}\n"
        f"Colors: {colors}\n"
        f"Platform: {item.get('platform', 'Not specified')}\n\n"
